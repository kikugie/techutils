...
public class TransparencyBuffer {
	...
	private static void drawUntexturedQuad(DrawContext context, int x1, int x2, int y1, int y2, int z, float u1, float u2, float v1, float v2) {
		...
		bufferBuilder.vertex(posMat, (float) x1, (float) y1, (float) z).texture(u1, v1);
		bufferBuilder.vertex(posMat, (float) x1, (float) y2, (float) z).texture(u1, v2);
		bufferBuilder.vertex(posMat, (float) x2, (float) y2, (float) z).texture(u2, v2);
		bufferBuilder.vertex(posMat, (float) x2, (float) y1, (float) z).texture(u2, v1);
