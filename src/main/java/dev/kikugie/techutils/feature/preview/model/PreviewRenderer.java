...
public class PreviewRenderer {
	...
	public PreviewRenderer(LitematicaSchematic schematic, InteractionProfile profile) {
		...
		final var dimensions = mesh.getFullBox();
		this.xSize = (int) dimensions.getXLength();
		this.ySize = (int) dimensions.getYLength();
		this.zSize = (int) dimensions.getZLength();
	...
	private void drawIntoActiveFramebuffer(DrawContext context, int x, int y, int size) {
		...
		// Rotation
		modelViewStack.multiply(RotationAxis.POSITIVE_X.rotation((float) this.profile.slant()));
		modelViewStack.multiply(RotationAxis.POSITIVE_Y.rotation((float) this.profile.angle()));
		...
		RenderSystem.runAsFancy(() -> {
			...
			// --> Draw
			this.draw(modelViewStack.peek().getPositionMatrix());
		});
