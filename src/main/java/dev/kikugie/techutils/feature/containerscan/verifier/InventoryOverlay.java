...
public class InventoryOverlay {
	...
	public static MousePos mousePos = new MousePos();
	@Nullable
	...
	public static boolean setSlotToSchematicItem(Slot slot) {
		...
		if (schematicItem.isEmpty()) {
			...
			var placeholder = ItemPredicateUtils.getPlaceholder(schematicItem);
			slot.setStack(placeholder != null ? placeholder : schematicItem);
			return true;
		}
	}
	...
	public ItemStack drawStackInternal(DrawContext context, Slot slot, ItemStack stack) {
		...
		int color = 0;
		var predicate = ItemPredicateUtils.getPredicate(schematicStack);
		if (predicate != null) {
			if (stack.isEmpty()) {
				color = this.MISSING_COLOR;
				var placeholder = ItemPredicateUtils.getPlaceholder(schematicStack);
				stack = placeholder != null ? placeholder : schematicStack;
		...
		} else if (LitematicConfigs.VERIFY_ITEM_NBT.getBooleanValue()
			&& !Objects.equals(schematicStack.getNbt(), stack.getNbt())) {
			color = WRONG_COLOR;
		}
